...
void ParticleSystem::Initialize(const ParticleSystemInfo& info, const CASTLEEngine::CEMath::Vector3& spawnPosition, const CASTLEEngine::CEMath::Vector3& spawnDirection, float delay)
{
    mNextAvailableParticleIndex = 0;
    mInfo = info;
...
void ParticleSystem::SpawnParticle()
{
    ...
    const Vector3 rotAngle = (Dot(mSpawnDirection, Vector3::YAxis) >= 0.99f) ? Normalize(Cross(Vector3::XAxis, mSpawnDirection)) : Normalize(Cross(Vector3::YAxis, mSpawnDirection));
    const Matrix4 matRot = Matrix4::RotationAxis(rotAngle, randomAngle);
