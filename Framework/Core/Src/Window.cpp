...
LRESULT CALLBACK WindowMessageHandler(HWND handle, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        case WM_DESTROY:
        {
            // Logic whenever you close a window. Things like auto-save
            PostQuitMessage(0);
            return 0;
        }
    }
...
void Window::Initialize(HINSTANCE instance, std::wstring appName, uint32_t width, uint32_t height)
{
    ...
    wcex.hIcon = LoadIcon(nullptr, IDI_APPLICATION); // Corner of the window
    ...
    // Every windows requries a window object
    // Dispatch messages
    // Register class and specifiy the type of window we want
