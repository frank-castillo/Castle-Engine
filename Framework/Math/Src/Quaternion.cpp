...
namespace CASTLEEngine::CEMath
{
    ...
    Quaternion Quaternion::CreateFromAxisAngle(const CEMath::Vector3& axis, float angle) noexcept
    {
        const float c = cos(angle * 0.5f);
        const float s = sin(angle * 0.5f);
        const CEMath::Vector3 a = CEMath::Normalize(axis);
    ...
    CASTLEEngine::CEMath::Quaternion Quaternion::LookRotation(const Vector3& lookAt, const Vector3& upDirection) noexcept
    {
        Vector3 forward = lookAt;
        Vector3 up = upDirection;
        Vector3 forwardNormalized = CASTLEEngine::CEMath::Normalize(forward);

        Vector3 right = CASTLEEngine::CEMath::Cross(up, forwardNormalized);
