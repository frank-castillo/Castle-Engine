...
void ExportEmbeddedTexture(const aiTexture* texture, const Arguments& args, const std::filesystem::path& fileName)
{
    ...
    {
        printf("Error: failed to laod open file %s for saving\n", fullFileName.c_str());
        return;
    }
...
uint32_t TryAddBone(const aiBone* bone, Skeleton& skeleton, BoneIndexLookUp& boneIndexMap)
{
    ...
    newBone->name = std::move(boneName); // Performance - Take all data and cleanig the bone name
    newBone->index = static_cast<int>(skeleton.bones.size()) - 1;
    newBone->offsetTransform = ToMatrix4(bone->mOffsetMatrix); // ToMatrix
                ...
                if (aiMesh->HasBones())
                {
                    ...
                    for (uint32_t b = 0; b < aiMesh->mNumBones; ++b)
                    {
                        ...
                        for (uint32_t w = 0; w < bone->mNumWeights; ++w)
                        {
                            ...
                            auto& count = numWeightsAdded[weight.mVertexId]; // Coutn of how many vertices we have updated

                            if (count < Vertex::MaxBoneWeights)
                            {
                                vertex.boneIndices[count] = boneIndex;
                                vertex.boneWeights[count] = weight.mWeight;
                                ++count; // This also increases numWeighrAdded as this is a refernce value
                            }
                        }
                    }
                }
